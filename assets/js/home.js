...
document.addEventListener('DOMContentLoaded', () => {                               // Nos aseguramos de que el DOM esté completamente cargado antes de ejecutar cualquier código para evitar errores relacionados con elementos que aún no existen en el DOM.
    ...
    if (contenedor) {                                                               // Verificamos que el contenedor exista antes de intentar modificarlo para evitar errores si el elemento no se encuentra en el DOM. Esto es especialmente importante si el código se ejecuta en diferentes páginas donde el contenedor podría no estar presente.
        fetch('novedades.json')                                                     // Realizamos una solicitud para obtener el archivo JSON que contiene la información de las novedades dinámicas. Asegúrate de que la ruta al archivo JSON sea correcta y que el archivo esté disponible en el servidor.
            .then(res => {                                                          // Verificamos que la respuesta de la solicitud sea exitosa antes de intentar procesar los datos. Si la respuesta no es exitosa (por ejemplo, si el archivo no se encuentra o hay un error en el servidor), lanzamos un error para manejarlo adecuadamente en el bloque catch.
            ...
            .then(datos => {                                                        // Una vez que tenemos los datos de las novedades dinámicas en formato JSON, iteramos sobre cada novedad para crear y mostrar su contenido dinámicamente en el contenedor seleccionado. Esto nos permitirá mostrar la información de cada novedad de manera atractiva y organizada utilizando tarjetas de Bootstrap.
                const columna = document.createElement('div');                      // Creamos un nuevo elemento div que servirá como columna para cada novedad dinámica. Esto nos permitirá organizar cada novedad en una estructura de columnas utilizando las clases de Bootstrap para asegurar que se vean bien en diferentes tamaños de pantalla.
                ...
                // Aquí se crea la estructura HTML de cada tarjeta utilizando template literals para insertar los datos de cada novedad dinámica, como la imagen, el título, la descripción y un botón para ver detalles. La tarjeta utiliza clases de Bootstrap para darle estilo y asegurarse de que se vea bien en diferentes dispositivos.
                columna.innerHTML = `                   
